...
export default class RtcEngine {
  ...
  private userData: Map<number, string> = new Map();
  private hostUid: number = 0;
  ...
  setUserData(uid: number, name: string) {
    this.userData.set(uid, name);
  }
  ...
  async joinChannel(
    ...
    this.client.on('user-joined', user => {
      ...
      this.sendLogToServer("JOIN", user.uid, timestamp);
    });
    ...
    this.client.on('user-left', user => {
      ...
      const timestamp = new Date().toISOString();
      this.sendLogToServer("LEAVE", user.uid, timestamp);
    });
  ...
  private sendLogToServer(action: string, uid: number | string, timestamp: string) {
    const attendeeName = this.userData.get(Number(uid)) || '';
    const hostName = this.userData.get(this.hostUid) || '';
    ...
    const logData = {
      ...
      uid,
      timestamp,
      meetingId: this.channelName,
      attendeeName,
      hostName,
      userAgent,
      platform,
      language,
      loggerSource: 'AgoraRTC-Engine'
    };
    ...
    .then(response => {
      if (!response.ok) {
        ...
        console.log(`✅ Log sent: ${action} for UID ${uid} (${attendeeName}) - Logger: AgoraRTC-Engine`);
      }
    })
    .catch(error => {
